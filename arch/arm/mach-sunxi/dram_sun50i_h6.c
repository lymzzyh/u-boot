...
unsigned long mctl_calc_size(struct dram_para *para)
{
	...
	return (1UL << (para->cols + para->rows + 3)) * 4 * para->ranks;
}
...
unsigned long sunxi_dram_init(void)
{
	...
	return get_ram_size((long int *)CONFIG_SYS_SDRAM_BASE, 0x80000000);
};
